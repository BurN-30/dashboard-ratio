/**
 * API Client pour TrackBoard
 * Se connecte au backend FastAPI.
 *
 * Modes :
 * - same-origin (prod, défaut) : NEXT_PUBLIC_API_URL non défini, les fetchs sont
 *   relatifs ("/api/...") et le reverse proxy (Caddy/Nginx) route vers le backend.
 * - cross-origin (dev local) : NEXT_PUBLIC_API_URL=http://localhost:8000 dans .env.local.
 * - démo : NEXT_PUBLIC_DEMO=true → données fictives, aucun appel réseau.
 */
...
// Vide = same-origin (relatif). Sinon URL absolue (dev cross-origin).
